...
def batch_predict(np_imgs):
    """Convert numpy images → tensors → model probabilities"""
    tensors = []
    for arr in np_imgs:
        pil = Image.fromarray(arr).convert("RGB")
        pil = pil.resize((224, 224))  # keep same size as training
...
async def explain_endpoint(file: UploadFile = File(...)):
    try:
        ...
        # Faster but good quality
        explanation = explainer.explain_instance(
            img_np,
            batch_predict,
            top_labels=1,
            num_samples=600,       # GOOD for local (fast + accurate)
            hide_color=0,
            random_seed=42
        )
        ...
        temp, mask = explanation.get_image_and_mask(
            ...
            positive_only=True,
            num_features=10,       # 8 superpixels → clear visualization
            hide_rest=False
        )
